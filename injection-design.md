...
├── current_draft           当前会话的草稿状态（蒸馏中但未发布）
│   └─ {
│       "article_id": UUID?,
│       "title": "草稿标题",
...
**会话与文章的关系**：
- 一个对话窗口默认对应**一篇**正在蒸馏的文章
- 用户二次 `=>` 蒸馏 → 视为对当前草稿的**补充和修改**
- 用户发布后 → 当前会话关闭蒸馏模式
- 架构预留支持单会话多篇文章的可能性（通过 `current_draft` 管理）
