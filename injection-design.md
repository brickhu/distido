...
- 用户回复「确认」→ POST 到 API 创建云端草稿
- 最终发布在 `app.distito.com` 确认后完成，不自动公开发布
- 一次对话默认对应**一篇**蒸馏中的文章

**创作方向询问**：

当对话涵盖多个话题时，Agent 应先问清楚创作方向，再开始蒸馏：

```
用户：=> 总结为文章

Agent：本次对话涉及以下几个方向，你想写哪篇？

  ○ 技术架构：CI/CD 流水线设计与最佳实践
  ○ 商业策略：技术团队的 DevOps 投入产出分析
  ○ 综合：上述两个方面都包含
...
③ 用户回复「确认」
         ...
         ↓
④ POST 到 API → 创建草稿（status = draft）
   └─ 反馈：「✅ 文章已保存为草稿，
            请到 app.distito.com/edit/<id> 确认发布。」

         ↓
⑤ 用户在 app.distito.com 上审阅 → 点击「发布」
   └─ status → published，builder 触发重建
```
...
输出原则（重要）：
- 用叙事风格写作，像一篇真实的经验分享，不是技术报告
- 开头用故事引入，让读者有代入感
- 避免表格罗列和结构化字段展示（那些是后台用，不给读者看）
- 融入个人反思和开放问题，让读者带走可复用的思考
- 对话涵盖多个话题时，先问用户想往哪个方向蒸馏
```

#### 蒸馏输出框架

当用户 `=> 总结发布` 时，按 What I Learned 框架组织内容：

```
# 标题（叙事型，吸引人）

💡 Key Takeaway（一句话精华，放在摘要位置）
...
### 3. 反思 Reflections
个人复盘、经验总结、做得对和做得不对的地方。

### 4. 开放问题 Open Questions
尚未解决、值得继续探索的方向。

🔗 Related（同 session 的其他知识，同 project 的相关知识）

---
*Distilled from a conversation about [topic].*
```

注意：结构化元数据（project、intent、injections、session_meta）只写入 API，不在正文中展示。
...
| 草稿保存成功 | `✅ 文章已保存为草稿，\n请到 app.distito.com/edit/{{id}} 确认发布。` |
| 发布成功 | `✅ 文章已发布 → https://distito.com/@{{user}}/{{slug}}` |
| 发布预览 | 见上方「蒸馏发布预览模板」 |
